...
(async()=>{
  for(const slug of slugs){
    const route=`shop/category/${slug}.html`;
    ...
    for(const match of html.matchAll(/href="\.\.\/medicine\/([^"]+\.html)"/g))if(!fs.existsSync(path.join('shop','medicine',match[1])))failures.push(`${route}: missing medicine target ${match[1]}`);
    const checks=[
      [/<h1\b[^>]*>[^<]+<\/h1>/i,'H1'],
      [/class="[^"]*\bcategory-layout\b[^"]*"/,'two-column layout'],
      [/id="category-select"/,'category dropdown'],
      [/class="category-product-grid"/,'medicine grid'],
      [/\.\.\/medicine\/[^"']+\.html/,'portable medicine links'],
      [/class="category-reading"/,'supporting content'],
      [new RegExp(`<link rel="canonical" href="https://www\\.midlandsrx\\.co\\.uk/shop/category/${slug}\\.html">`),'canonical']
    ];
    for(const [pattern,label] of checks)if(!pattern.test(html))failures.push(`${route}: missing ${label}`);
  }
  const legacy=[
    ['shop/category/adhd-%26-wakefulness.html','adhd-and-wakefulness.html'],
    ['shop/category/sleep-%26-insomnia-medication.html','sleep-and-insomnia-medication.html'],
    ['shop/category/nerve-pain-%26-anxiety-related-medicines.html','nerve-pain-and-anxiety-related-medicines.html']
  ];
  ...
  for(const slug of slugs)if(!sitemap.includes(`<loc>https://www.midlandsrx.co.uk/shop/category/${slug}.html</loc>`))failures.push(`sitemap missing ${slug}`);
