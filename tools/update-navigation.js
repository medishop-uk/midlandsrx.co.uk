...
function dropdown(prefix) {
  return `<details class="nav-categories"><summary>All Categories <span aria-hidden="true">⌄</span></summary><div class="nav-category-menu">${categories.map(x=>`<a href="${prefix}shop/category/${x[3]}.html">${x[0]}</a>`).join('')}</div></details>`;
}

function updatePagesJs() {
  const file=path.join(root,'assets','js','pages.js');
  let js=fs.readFileSync(file,'utf8');
  const replacement=`function header(){const active=x=>(kind.startsWith(x)||(kind==='product'&&x==='shop'))?' class="active"':'';return \`<a class="page-skip" href="#page-content">Skip to content</a><div class="page-notice"><div class="page-container"><p>Private UK-wide delivery</p><p>Prescription medicines require a valid prescription · <a href="\${root}/#how-it-works">How requests work</a></p></div></div><header class="page-header"><div class="page-container page-header-inner"><a class="page-brand" href="\${root}/"><img src="\${root}/assets/img/logo.svg" alt="MidlandsRx"></a><nav class="page-nav">${dropdown('${root}/')}<a\${active('shop')} href="\${root}/shop/">Shop</a><a\${active('blog')} href="\${root}/blog/">Blog</a><a\${active('about')} href="\${root}/about-us/">About Us</a><a href="\${root}/#contact">Contact Us</a></nav><div class="page-actions"><a class="page-cta" href="\${whatsapp}" target="_blank" rel="noopener">Contact on WhatsApp</a><button class="page-menu" aria-label="Open menu" aria-expanded="false"><i></i><i></i><i></i></button></div></div><nav class="page-mobile-nav"><a href="\${root}/#categories">All Categories</a><a href="\${root}/shop/">Shop</a><a href="\${root}/blog/">Blog</a><a href="\${root}/about-us/">About Us</a><a href="\${root}/#contact">Contact Us</a></nav></header>\`}`;
...
function updateHomepage() {
  ...
  html=html.replace(/<div class='header-actions'>[\s\S]*?<button class='menu-button'/,`<div class='header-actions'><button class='search-jump' aria-label='Search medicines'>Search</button><button class='basket-button' data-cart-open>Basket <b data-cart-count>0</b></button><button class='menu-button'`);
...
function updateHomepageCatalog() {
  ...
  js=js.replace(/function renderCategories\(\)\{[^\n]+/,`function renderCategories(){catGrid.innerHTML=categories.map((x,i)=>'<a class="category-card" href="shop/category/'+x[3]+'.html"><span class="category-icon">'+x[2]+'</span><h3>'+x[0]+'</h3><p><span>'+listedMedicines.filter(m=>m.c.includes(i)).length+' medicines</span><b>→</b></p></a>').join('');filters.innerHTML='<button class="filter-chip active" data-category="all">All medicines</button>'+categories.map((x,i)=>'<button class="filter-chip" data-category="'+i+'">'+x[1]+'</button>').join('')}`);
  fs.writeFileSync(file,js,'utf8');
}
...
function updateProgressiveNavigation() {
  ...
  for(const file of files){
    ...
    const nav=`<nav data-source-navigation aria-label="Primary navigation"><details><summary>All Categories</summary>${categories.map(x=>`<a href="${prefix}shop/category/${x[3]}.html">${x[0]}</a>`).join('')}</details><a href="${prefix}shop/">Shop</a> <a href="${prefix}blog/">Blog</a> <a href="${prefix}about-us/">About Us</a> <a href="${prefix}#contact">Contact Us</a></nav>`;
